...
const HeaderContainer = tw.div`h-20 w-full shadow-md fixed top-0 z-40 flex justify-between`;
...
const RenderNavItems = () => {
  ...
  console.log(cookies);
  const navTopConfigsFiltered = navTopConfigs.filter(
    (item) => cookies?.userInfo.role === "admin" || item.id !== "User"
  );
  ...
  return navTopConfigsFiltered.map((x, index) => {
    const isActive = !!x.item.find((x) => pathName.indexOf(x.navLink) >= 0);
    return (
      <ItemContainer
        className="group"
        onMouseMove={() => handleMouseMove(index, true)}
        onMouseLeave={() => handleMouseMove(index, false)}
        key={index}>
        <LinkItem
          isActive={isActive || !pathName.indexOf(x.navLink)}
          onClick={() => x.item.length === 0 && history(x.navLink)}>
          {x.id}
        </LinkItem>
